...
def arr2node(gra, data, state_dim=2, action_dim=1):
    state = (tuple(data[0:state_dim]), tuple(data[state_dim, state_dim * 2]))
    if state in gra.keys():
        return gra[state]
    raise ValueError("数据量太少")
#TODO: 返回值，随机的对应不上怎么办
#TODO: distance()调用，返回后的两个节点
...
class CustomKMeans(KMeans):
    ...
    def distance(self, x, y, state_dim=2, action_dim=1):
        # 判断两个状态是否相同
        x = arr2node(graph, x)
        y = arr2node(graph, y)
        if x.state == y.state:
...
#   node2arr需要指出dim， state, action, reward,
#   后续节点的问题 也调用nodearr吗
if __name__ == '__main__':
    ...
    # distance = kmeans.transform(graph)
    # print(distance)
    #   计算自定义距离矩阵
    # distance = kmeans.distance(graph[(-0.5, -0.4), (-0.1, 0.0)], graph[(-0.5, -0.4), (0, 0.1)])
    # print(distance)
    print("成功！！")
