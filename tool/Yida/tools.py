...
class data_prcss:
    def __init__(self, col_num, state_dim, act_dim, rewd_dim, cost_dim,
                 state_prec, act_prec, rewd_prec, cost_prec,
    ...
    # 得到 状态id/动作id的值 给定具体数据，得到id，根据id可以判断属于哪个状态 id（1，2，3） 和状态的维度一致
    # 输入 状态值（s1，s2，s3），状态粒度（0.01,0.01,0.01) -> 输出：(1,2,3)
    ...
    # 输入抽象状态id （1，2，3） 输出：（第一维的中心点，第二维的中心点，第三维的中心点）
...
if __name__ == '__main__':
    data = data_prcss(9, 2, 1, 1, 1,
                      (1e-3, 1e-3), (1e-3,), (1e-3,), (1e-3,),
                      (1e-1, 1e-1), (1e-2,), (1e-2,), (1e-2,),
                      (1, 1), (-1, -1), (1,), (-1,),
                      (2,), (0,), (100,), (0,))
